...
@WebServlet(urlPatterns = "/SimpleRequestReply")
public class SimpleRequestReply extends HttpServlet {
  private static final long serialVersionUID = 20130522L;
    
  @Resource(name = "jms/RequestQueue")
  private Queue requestQueue;
  
  @Resource(name = "jms/SimpleReplyQueue")
  private Queue simpleReplyQueue;
  ...
  /** Logger */
  private static final WLMJMSLogger log = new WLMJMSLogger(SimpleRequestReply.class);
  
  /**
   * @see HttpServlet#HttpServlet()
   */
  public SimpleRequestReply() {
    super();
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      out.println("Reply destination: " + simpleReplyQueue);
      message.setJMSReplyTo(simpleReplyQueue);
      ...
      // to think carefully about how to handle timeout scenarios.
      ...
      wlmConnection.getProducer().send(message, 
          ...
          timeToLive);
      String requestMessageID = message.getJMSMessageID();
      out.println("JMSMessageID: " + requestMessageID);
      out.flush();
      
      // Create our consumer for the reply
      String selector = "JMSCorrelationID = '" + requestMessageID + "'";
      MessageConsumer consumer = wlmConnection.getSession().createConsumer(simpleReplyQueue, selector);
      wlmConnection.getConnection().start();
      
      // Listen for a reply, up to the expiration time we specified on the request
      long receiveStartTime = System.currentTimeMillis();
      Message replyMessage = consumer.receive(timeToLive);
